...
2. **Subagent Dispatch**: For each identified item, invoke a fresh subagent with its own clean context. Pass only:
   ...
   - **Mandatory AST Manifest Injection**: The explicit, pre-extracted SysML v2 AST slice from `.pipeline/schema.sysml`:
     * For Epics/Features: The exact AST package and part def / item def definition with all owned properties, ports, operations, and constraints.
     * For User Stories: The exact AST action def, port def, and complete state def state machine family (all states, events, transitions, guards, and entry/exit actions).
     * For Use Cases: The exact AST use case def with subject part def, actor port contracts, and include/extend dependencies.
...
1. **Trigger / Dispatch**: The Coordinator MUST invoke a fresh subagent (TypeName: `self`, Role: `Normative Research Worker`) with `.pipeline/schema.sysml`, the extracted AST `part def` component manifest, the 4 Universal Failure Dimensions (Interface, State, Action, Resource), target domain/protocol requirements, applicable regulatory standards list, and standard template paths (`skills/spec-orchestrator/resources/RESEARCH_INVENTORY_CANONICAL_TEMPLATE.md`), appending the keyword `PROCEED` to authorize execution.
2. **Normative & Component Failure Mode Research Protocol**:
   ...
   - **AST Component Failure Mode Research Swarm Protocol (Step 1.5: Technology Stack & Architecture Research)**:
     1. Parse `schema/*.sysml` / `schema/*.yang` (or `.pipeline/schema.sysml`) and extract all declared `part def` nodes $\{P_1, P_2, \dots, P_N\}$.
     2. Dispatch $N$ context-isolated research workers, passing each worker its respective component AST fragment (name, ports, actions, operations, states, constraints).
     3. Instruct workers to evaluate the 4 Universal Failure Dimensions (Interface, State, Action, Resource) and assign Severity ($S \in [1, 10]$), Occurrence ($O \in [1, 10]$), Detection ($D \in [1, 10]$), and explicit `SSOT`/`Derived` basis tags.
     4. Compile output into intermediate artifact `docs/research/FAILURE_MODE_REGISTRY.md`.
   - **Mandatory Output Artifacts**:
     ...
     * **Component Failure Mode Registry** (`docs/research/FAILURE_MODE_REGISTRY.md`): Authoritative multi-mode component failure registry covering 100% of declared AST parts with at least 3 distinct failure modes per part across the 4 universal failure dimensions, S/O/D ratings in [1, 10], integer RPN calculations ($RPN = S \times O \times D$), and explicit `SSOT` or `Derived` basis classifications.
...
4. **Wait & Verify**: The Coordinator waits for the subagent to report completion, reads its final report, and:
   a. Verifies that `docs/research/RESEARCH_INVENTORY.md` and `docs/research/FAILURE_MODE_REGISTRY.md` exist and are fully populated with zero placeholder tokens.
   b. Performs a sampling audit (`view_file`) on `docs/research/RESEARCH_INVENTORY.md` and `docs/research/FAILURE_MODE_REGISTRY.md` to confirm all cited standards, clauses, component failure modes, and population register entries conform to the mandatory traceability and AST coverage rules.
5. **Validation Gate**: Verify that `docs/research/RESEARCH_INVENTORY.md` and `docs/research/FAILURE_MODE_REGISTRY.md` are committed and contain a complete Declared-Total Population Register with 100% public clause citations and 100% AST component failure mode coverage ($\ge 3$ distinct modes per part). Once this validation passes, **execute Phase 0.75 immediately without pausing for user approval.**

## Phase 0.75: Hierarchical ConOps & Mission Intent Tree Engineering (Worker ConOps)
1. **Trigger / Dispatch**: The Coordinator MUST invoke a fresh subagent (TypeName: `self`, Role: `ConOps & Mission Intent Tree Worker`) with the `spec-conops-engineering` skill, paths to input schemas (`schema/`, `.pipeline/schema.sysml`), the explicit manifest of all AST `state def` prefix families ($\ge 2$ states) and `part def` nodes, `docs/research/RESEARCH_INVENTORY.md`, `docs/research/FAILURE_MODE_REGISTRY.md`, FMECA failure modes, SORA risk classes, and user operational intent, appending the keyword `PROCEED` to authorize execution.
2. **Execution**: The `ConOps & Mission Intent Tree Worker` subagent ingests operational intent, system architecture, `docs/research/FAILURE_MODE_REGISTRY.md`, SORA risk profiles, and `docs/research/RESEARCH_INVENTORY.md`. In Step 2 (Safety Matrix Assembly), it assembles Section 6.1 (Stateflow Synthesis Hooks & Safety Statecharts) and Section 7 (FMECA Criticality Matrix) of `docs/safety/STPA_MATRIX.md` directly from `schema/*.sysml` AST `state def` nodes and `docs/research/FAILURE_MODE_REGISTRY.md`. It mandates that every declared AST state machine prefix family with $\ge 2$ states is represented with a dedicated Mermaid `stateDiagram-v2` figure and referenced in the Section 6.1 Stateflow synthesis hooks. It also ensures 100% representation of all AST `part def` nodes, a minimum of 3 distinct failure modes per component across the 4 universal failure dimensions, integer RPN ($RPN = S \times O \times D$), and explicit `SSOT`/`Derived` basis annotations. It extracts discrete units adhering to `.pipeline/schemas/conops_specification_schema.json` (12 canonical units under `docs/conops/units/conops/`) and `.pipeline/schemas/mission_intent_specification_schema.json` (10 canonical units under `docs/conops/units/mission_intent/`). It enforces pure open schema generation ($N \ge N_{\mathrm{min}}$) with zero static row caps, open multi-domain threat taxonomy (Kinetic, Mechanical, Environmental, EW/Cyber, Power/Thermal, Optical, Human), KaTeX math formulas, and 100% public clause citations. It executes deterministic modular assembly via `python3 scripts/assemble_conops.py --input-dir docs/conops/units/ --output-dir docs/conops/`, validating unit integrity, TOC generation, and internal links. Registers the ConOps suite under the `conops` issue label using `./skills/spec-orchestrator/scripts/create_issue.sh`, runs immediate verification checks to confirm the tracker body is fully populated, and commits/pushes the changes.
...
1. **Trigger / Dispatch**: The Coordinator MUST invoke a fresh subagent (TypeName: `self`, Role: `Interface Spec Worker`) with the `spec-icd-engineering` skill, `.pipeline/schema.sysml`, and the extracted AST `port def`, `connection`, `interface def`, and `item flow` manifest slices, appending the keyword `PROCEED` to authorize execution.
...
1. **Trigger / Dispatch**: The Coordinator MUST invoke a fresh subagent (TypeName: `self`, Role: `Behavioral Spec Worker`) with the `spec-user-story-engineering` skill, `.pipeline/schema.sysml`, the extracted AST `action def`, `port def`, and complete `state def` state machine family slices, and the text/path of the target specification document, appending the keyword `PROCEED` to authorize execution.
2. **Execution**: The `Behavioral Spec Worker` subagent parses the SysML v2 AST model (`action def`, `state def`, and `port def` nodes) and operational scenarios to identify required User Stories (including calculations and transitions). It dispatches a fresh context-isolated subagent for each User Story to write its specification file, injecting the mandatory AST action, port, and state machine manifest slices. Before committing, pushing, or creating issues, it MUST execute the local validation check (`./skills/spec-orchestrator/scripts/verify_model_coverage.py --spec-only --allow-missing-specs --only "<local-md-file>"`) and fix all reported errors until the linter passes with exit code 0. The subagent registers the User Stories in the tracker using `./skills/spec-orchestrator/scripts/create_issue.sh "<local-md-file>" "user-story" "<Extract_Title_From_YAML_Metadata>"`, runs immediate verification check (`gh issue view <ID> --json body`) to ensure their bodies are fully populated in the tracker, and commits/pushes the changes.
3. **Wait & Verify**: The Coordinator waits for the subagent to report completion, reads its final report, and:
   a. Query the `git diff` to identify the generated file paths.
   b. Run a file read check (`view_file`) on a random sample (at least 1-2 files) of the newly generated files to verify formatting compliance (such as BDD syntax, UML diagrams format).
   c. Run the linter locally over the newly added files to double-check that the validation gate is fully satisfied:
