...
1. **Trigger / Dispatch**: The Coordinator MUST invoke a fresh subagent (TypeName: `self`, Role: `ConOps & Mission Intent Tree Worker`) with the `spec-conops-engineering` skill and paths to input schemas (`schema/`, `.pipeline/schema.sysml`), `docs/research/RESEARCH_INVENTORY.md`, `docs/research/FAILURE_MODE_REGISTRY.md`, FMECA failure modes, SORA risk classes, and user operational intent, appending the keyword `PROCEED` to authorize execution.
