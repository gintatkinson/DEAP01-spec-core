...
- **Environmental Envelope:** Operating ambient atmospheric pressure down to $P_{\mathrm{op\_min}}$ (`{{M500_OP_PRESSURE_KPA}}` kPa, equivalent to an operational altitude ceiling $h_{\mathrm{alt\_max}}$ of `{{M500_OP_ALTITUDE_M}}` m above sea level); unpowered storage/transit pressure down to $P_{\mathrm{store\_min}}$ (`{{M500_STORE_PRESSURE_KPA}}` kPa, equivalent to a cargo hold ceiling $h_{\mathrm{store\_max}}$ of `{{M500_STORE_ALTITUDE_M}}` m). Rapid decompression rate $\Delta P / \Delta t \le \dot{P}_{\mathrm{decomp\_max}}$ (`{{M500_DECOMPRESSION_RATE_KPA_S}}` kPa/s).
- **Exposure Duration:** Minimum chamber dwell duration $t_{\mathrm{dwell}} \ge \tau_{\mathrm{alt\_dwell\_min}}$ (`{{M500_DWELL_DURATION_HR}}` hr) following chamber pressure stabilization; rapid decompression transition execution time $\Delta t \le \tau_{\mathrm{decomp\_time}}$ (`{{M500_DECOMPRESSION_TIME_S}}` s).
...
- **Environmental Envelope:** Continuous operational exposure up to $T_{\mathrm{op\_high\_max}}$ (`{{M501_OP_HIGH_TEMP_C}}` °C) under induced solar/ambient thermal loads; unpowered storage temperature up to $T_{\mathrm{store\_high\_max}}$ (`{{M501_STORE_HIGH_TEMP_C}}` °C) under Hot Dry ($A_1$) and Basic Hot ($A_2$) climatic cycles. Maximum rate of temperature rise $\dot{T}_{\mathrm{rise}} \le \dot{T}_{\mathrm{rise\_max}}$ (`{{M501_TEMP_RISE_RATE_C_MIN}}` °C/min).
- **Exposure Duration:** Storage: continuous exposure across $N_{\mathrm{store\_cycles}}$ (`{{M501_STORE_CYCLE_COUNT}}` 24-hr diurnal cycles, minimum `{{M501_STORE_TOTAL_HR}}` hr); Operational: continuous exposure for $t_{\mathrm{op\_high\_duration}}$ (`{{M501_OP_DURATION_HR}}` hr) post core thermal stabilization.
- **Operational Functional Checks:** Continuous processor junction temperature monitoring ($T_{\mathrm{junction}} \le T_{\mathrm{junction\_max}}$), control loop execution rate tracking ($f_{\mathrm{control}} \ge f_{\mathrm{control\_nominal}}$), power converter efficiency logging, actuator torque output measurement under peak thermal loading, and communications transceiver packet error rate monitoring.
...
- **Environmental Envelope:** Continuous operational exposure down to $T_{\mathrm{op\_low\_min}}$ (`{{M502_OP_LOW_TEMP_C}}` °C); unpowered storage temperature down to $T_{\mathrm{store\_low\_min}}$ (`{{M502_STORE_LOW_TEMP_C}}` °C) under Cold ($C_1$) and Severe Cold ($C_2$) climatic classifications.
- **Exposure Duration:** Storage soak duration $t_{\mathrm{cold\_soak}} \ge \tau_{\mathrm{cold\_soak\_min}}$ (`{{M502_STORAGE_SOAK_HR}}` hr); Operational cold soak $t_{\mathrm{cold\_op}} \ge \tau_{\mathrm{cold\_op\_min}}$ (`{{M502_OP_SOAK_HR}}` hr) post temperature equilibrium.
- **Operational Functional Checks:** Cold-start Built-In-Test (BIT) initiation within $t_{\mathrm{start}} \le \tau_{\mathrm{cold\_start\_max}}$ (`{{M502_COLD_START_TIME_S}}` s) at $T_{\mathrm{op\_low\_min}}$; dynamic actuator break-away torque and slew rate verification; energy storage internal impedance and discharge curve validation; sensor bias drift and oscillator frequency stability logging.
- **Acceptance Criteria:** Full cold-start initialization and transition to nominal execution without external preheating; clock oscillator frequency drift within timing budget ($\Delta f / f_0 \le \epsilon_{\mathrm{clk\_max}}$); zero cracking, embrittlement, or loss of resilience in elastomeric seals, cabling jackets, and structural mounts; nominal lubricant viscosity and actuator response without overcurrent fault trips.
...
- **Environmental Envelope:** Rapid extreme temperature transition across the thermal shock gradient $[\mathbf{T}_{\mathrm{shock\_low}}, \mathbf{T}_{\mathrm{shock\_high}}]$ (`[{{M503_SHOCK_LOW_TEMP_C}} °C, {{M503_SHOCK_HIGH_TEMP_C}} °C]`), with maximum physical chamber transfer duration $t_{\mathrm{transfer}} \le \tau_{\mathrm{transfer\_max}}$ (`{{M503_MAX_TRANSFER_TIME_S}}` s).
- **Exposure Duration:** Minimum $N_{\mathrm{shock\_cycles}}$ (`{{M503_SHOCK_CYCLE_COUNT}}` complete shock cycles); dwell time at each extreme plateau $t_{\mathrm{dwell}} \ge \tau_{\mathrm{shock\_dwell}}$ (`{{M503_SHOCK_DWELL_HR}}` hr) to ensure complete thermal stabilization of internal structural core mass.
...
- **Acceptance Criteria:** Zero micro-cracking, delamination, or void formation across printed circuit boards, multi-layer ceramic capacitors, and solder joints; zero hermetic seal failure or internal enclosure moisture condensation; dimensional stability of optical and mechanical sensor mounting alignments within tolerance ($\Delta \theta_{\mathrm{align}} \le \theta_{\mathrm{tol\_max}}$).
...
- **Environmental Envelope:** Peak simulated solar spectral irradiance $I_{\mathrm{solar\_peak}} \le I_{\mathrm{solar\_max}}$ (`{{M505_PEAK_IRRADIANCE_W_M2}}` W/m², spectral distribution encompassing UV-A, UV-B, visible, and infrared wavelengths per MIL-STD-810H Table 505.7-I) under continuous chamber ambient air temperature up to $T_{\mathrm{solar\_amb}}$ (`{{M505_SOLAR_AMB_TEMP_C}}` °C).
- **Exposure Duration:** Procedure I: minimum $N_{\mathrm{solar\_cycles}}$ (`{{M505_DIURNAL_CYCLE_COUNT}}` continuous 24-hr diurnal heating cycles); Procedure II: continuous actinic exposure for $t_{\mathrm{actinic}} \ge \tau_{\mathrm{actinic\_min}}$ (`{{M505_ACTINIC_EXPOSURE_HR}}` hr).
- **Operational Functional Checks:** Continuous monitoring of internal enclosure internal thermal rise ($\Delta T_{\mathrm{internal}} \le \Delta T_{\mathrm{internal\_max}}$), optical perception window transmissivity measurement, surface coating reflectance evaluation, and telemetry health stream verification during peak irradiance.
- **Acceptance Criteria:** Internal compartment temperature rise constrained within thermal design margins (`{{M505_MAX_INTERNAL_TEMP_RISE_C}}` °C); zero chalking, blistering, peeling, or photolytic embrittlement of exterior polymers, radomes, and seal materials; optical transmissivity degradation across sensor covers $\Delta \text{Trans} \le \Delta \text{Trans}_{\mathrm{max}}$ (`{{M505_MAX_TRANSMISSIVITY_LOSS_PCT}}`%).
...
- **Environmental Envelope:** Precipitation rate $R_{\mathrm{precip}} \ge R_{\mathrm{precip\_req}}$ (`{{M506_PRECIP_RATE_MM_HR}}` mm/hr / `{{M506_PRECIP_RATE_IN_HR}}` in/hr) with accompanying horizontal wind velocity $v_{\mathrm{wind}} \ge v_{\mathrm{wind\_req}}$ (`{{M506_WIND_VELOCITY_M_S}}` m/s / `{{M506_WIND_VELOCITY_MPH}}` mph); water droplet diameter distribution $d_{\mathrm{droplet}} \in [0.5\text{ mm}, 4.5\text{ mm}]$ under nozzle water pressure $P_{\mathrm{nozzle}}$ (`{{M506_NOZZLE_PRESSURE_KPA}}` kPa).
- **Exposure Duration:** Minimum exposure duration $t_{\mathrm{exposure}} \ge \tau_{\mathrm{rain\_face\_duration}}$ (`{{M506_FACE_EXPOSURE_MIN}}` min per exposed face across all orthogonal axes, total exposure $t_{\mathrm{total}} \ge \tau_{\mathrm{rain\_total\_min}}$ of `{{M506_TOTAL_EXPOSURE_MIN}}` min).
...
- **Environmental Envelope:** Relative humidity $\text{RH} \ge \text{RH}_{\mathrm{aggravated}}$ (`{{M507_AGGRAVATED_RH_PCT}}`% ± 4% RH) across cyclic thermal profile $[T_{\mathrm{hum\_low}}, T_{\mathrm{hum\_high}}]$ (`[{{M507_HUMID_LOW_TEMP_C}} °C, {{M507_HUMID_HIGH_TEMP_C}} °C]`).
- **Exposure Duration:** Aggravated cyclic exposure spanning $N_{\mathrm{humid\_cycles}}$ (`{{M507_HUMID_CYCLE_COUNT}}` continuous 24-hr cycles, total exposure duration $t_{\mathrm{humid\_total}} \ge \tau_{\mathrm{humid\_total\_min}}$ of `{{M507_TOTAL_HUMID_HOURS}}` hr).
- **Operational Functional Checks:** Operational checkouts conducted at high-temperature / high-humidity plateau during alternating cycles (e.g., cycles 2, 5, 8, and 10); high-voltage dielectric breakdown test; post-exposure operational baseline verification within $t_{\mathrm{post}} \le \tau_{\mathrm{post\_humid\_check}}$ (`{{M507_POST_CHECK_HOURS}}` hr) of chamber egress.
- **Acceptance Criteria:** Zero electrical short circuits, dielectric breakdown, or insulation leakage ($R_{\mathrm{ins}} \ge R_{\mathrm{ins\_min}}$); zero micro-corrosion on gold/nickel-plated connector pins; zero delamination or dendritic conductive growth on conformal-coated printed circuit assemblies; zero fungal growth or degradation of potting materials per MIL-STD-810H Method 508.8.
...
- **Environmental Envelope:** Saline atomization solution concentration $C_{\mathrm{salt}}$ (`{{M509_SALT_CONCENTRATION_PCT}}`% ± 1% NaCl by weight); chamber exposure temperature $T_{\mathrm{chamber}}$ (`{{M509_CHAMBER_TEMP_C}}` °C ± 2 °C); fallout collection rate $1.0\text{ to }3.0\text{ ml}/80\text{ cm}^2/\text{hr}$; solution pH maintained within $6.5 \le \text{pH} \le 7.2$.
- **Exposure Duration:** Standard accelerated 48-hour continuous salt spray exposure followed by 48-hour controlled drying cycle at $+35\text{ °C}$ with relative humidity $\text{RH} \le 50\%$, repeated for $N_{\mathrm{salt\_cycles}}$ (`{{M509_SALT_CYCLE_COUNT}}` complete 96-hr cycles, total exposure $t_{\mathrm{salt\_total}} \ge \tau_{\mathrm{salt\_total\_min}}$ of `{{M509_TOTAL_EXPOSURE_HOURS}}` hr).
- **Operational Functional Checks:** Pre-test baseline functional pass, post-drying operational checkout following each 48-hr drying cycle, electrical bonding and grounding path resistance measurement ($R_{\mathrm{bond}} \le R_{\mathrm{bond\_max}}$), and mechanical latch/fastener operation check.
- **Acceptance Criteria:** Zero pitting or galvanic corrosion penetrating protective base metal coatings (hardcoat anodization, passivated stainless steel, electroless nickel); zero binding of exposed rotating joints, hinges, or bearing seals; electrical bonding resistance across structural chassis elements remains $R_{\mathrm{bond}} \le R_{\mathrm{bond\_limit}}$ (`{{M509_MAX_BONDING_RESISTANCE_MOHM}}` mΩ); full functional test pass.
...
- **Environmental Envelope:**
  - *Blowing Dust:* Particulate concentration $C_{\mathrm{dust}}$ (`{{M510_DUST_CONCENTRATION_G_M3}}` g/m³ ± 7 g/m³, Red China Clay / ISO 12103-1 A2 fine test dust) at air velocity $v_{\mathrm{dust}}$ (`{{M510_DUST_VELOCITY_M_S}}` m/s) across ambient (+23 °C) and high-temperature (+60 °C) plateaus.
  - *Blowing Sand:* Particulate concentration $C_{\mathrm{sand}}$ (`{{M510_SAND_CONCENTRATION_G_M3}}` g/m³, silica sand grain distribution $150\ \mu\text{m} \le d_{\mathrm{grain}} \le 850\ \mu\text{m}$) at air velocity $v_{\mathrm{sand}}$ (`{{M510_SAND_VELOCITY_M_S}}` m/s).
- **Exposure Duration:** Blowing Dust: $t_{\mathrm{dust}} \ge \tau_{\mathrm{dust\_duration}}$ (`{{M510_DUST_DURATION_HR}}` hr at $+23\text{ °C}$ plus `{{M510_DUST_HIGH_TEMP_DURATION_HR}}` hr at $+60\text{ °C}$ per face orientation); Blowing Sand: $t_{\mathrm{sand}} \ge \tau_{\mathrm{sand\_duration}}$ (`{{M510_SAND_DURATION_MIN}}` min per exposed face).
...
- **Acceptance Criteria:** Zero particulate ingress through IP6x certified labyrinth seals, connector gaskets, and hydrophobic vents; zero jamming, binding, or abrasive seizure of dynamic mechanical linkages; optical window transmission degradation $\Delta \text{Trans} \le \Delta \text{Trans}_{\mathrm{sand\_max}}$ (`{{M510_MAX_OPTICAL_DEGRADATION_PCT}}`%); nominal sensor SNR and motor RPM tracking.
...
- **Environmental Envelope:** Power Spectral Density (PSD) broadband random vibration profile $S_{\mathrm{vib}}(f)$ across frequency spectrum $f \in [20\text{ Hz}, 2000\text{ Hz}]$ with overall root-mean-square acceleration $G_{\mathrm{rms\_op}}$ (`{{M514_OP_VIBRATION_G_RMS}}` G_rms) for operational qualification and $G_{\mathrm{rms\_endurance}}$ (`{{M514_ENDURANCE_VIBRATION_G_RMS}}` G_rms) for structural endurance across three orthogonal axes ($X, Y, Z$).
- **Exposure Duration:** Operational test: $t_{\mathrm{vib\_op}} \ge \tau_{\mathrm{vib\_op\_axis}}$ (`{{M514_OP_AXIS_DURATION_HR}}` hr per axis); Structural endurance test: $t_{\mathrm{vib\_endurance}} \ge \tau_{\mathrm{vib\_endurance\_axis}}$ (`{{M514_ENDURANCE_AXIS_DURATION_HR}}` hr per axis, total duration across 3 axes $t_{\mathrm{total}} \ge \tau_{\mathrm{vib\_total\_min}}$ of `{{M514_TOTAL_VIBRATION_HR}}` hr).
...
- **Acceptance Criteria:** Structural integrity maintained with zero resonant fatigue cracking, loose hardware, or structural plastic deformation; fastener torque retention $\ge \tau_{\mathrm{torque\_retention\_min}}$ (`{{M514_MIN_FASTENER_TORQUE_RETENTION_PCT}}`%); state estimation error bounded by $e_{\mathrm{state}} \le \epsilon_{\mathrm{vib\_max}}$ (`{{M514_MAX_STATE_ERROR_MM}}` mm / `{{M514_MAX_ATTITUDE_ERROR_DEG}}` deg); zero intermittent signal dropouts or contact chatter exceeding $\tau_{\mathrm{chatter\_max}}$ (`{{M514_MAX_CONTACT_CHATTER_US}}` µs).
...
- **Environmental Envelope:**
  - *Functional Shock:* Terminal-peak sawtooth or half-sine acceleration pulse with peak amplitude $a_{\mathrm{shock\_peak}}$ (`{{M516_FUNCTIONAL_SHOCK_G}}` g) across pulse duration $\Delta t_{\mathrm{shock}}$ (`{{M516_SHOCK_DURATION_MS}}` ms, velocity change $\Delta v_{\mathrm{shock}} \ge \Delta v_{\mathrm{req}}$ of `{{M516_SHOCK_DELTA_V_M_S}}` m/s).
  - *Crash Hazard Shock:* Peak amplitude $a_{\mathrm{crash\_peak}}$ (`{{M516_CRASH_SHOCK_G}}` g) across pulse duration $\Delta t$ (`{{M516_CRASH_DURATION_MS}}` ms).
  - *Transit Drop:* Free-fall drop height $h_{\mathrm{drop}}$ (`{{M516_TRANSIT_DROP_HEIGHT_M}}` m / `{{M516_TRANSIT_DROP_HEIGHT_IN}}` inches) onto high-density plywood backed by concrete across all corners, edges, and faces.
...
- **Environmental Envelope:** Water droplet atomization spray at water temperature $T_{\mathrm{water}} \approx 0\text{ °C}$ under chamber air temperature $T_{\mathrm{ice\_amb}} \in [-10\text{ °C}, -2\text{ °C}]$ (`[{{M521_ICE_LOW_TEMP_C}} °C, {{M521_ICE_HIGH_TEMP_C}} °C]`); glaze or rime ice accretion thickness $\delta_{\mathrm{ice}} \ge \delta_{\mathrm{ice\_req}}$ (`{{M521_ICE_ACCRETION_THICKNESS_MM}}` mm / `{{M521_ICE_ACCRETION_THICKNESS_IN}}` inches).
- **Exposure Duration:** Water spray exposure duration until declared ice accretion thickness is achieved, followed by cold soak stabilization duration $t_{\mathrm{soak}} \ge \tau_{\mathrm{ice\_soak\_min}}$ (`{{M521_ICE_SOAK_HR}}` hr) at $-10\text{ °C}$.
...
- **Radiated Susceptibility (RS103):** Withstands High-Intensity Radiated Fields (HIRF) up to $E_{\mathrm{field}}$ (`{{EMC_RS103_FIELD_STRENGTH_V_M}}` V/m) across frequency spectrum $f \in [2\text{ MHz}, 40\text{ GHz}]$ per MIL-STD-461G Method RS103 without processor resets or telemetry corruption.
...
- **External Reference Signal Denial Resilience:** Capable of maintaining autonomous closed-loop state estimation and boundary containment for up to $t_{\mathrm{denied}} \ge \tau_{\mathrm{denied\_max}}$ (`{{SIGNAL_DENIAL_MAX_DURATION_S}}` s) of continuous external positioning signal loss via dead reckoning and kinematic state observers with spatial drift rate bounded by $\text{Drift} \le \text{Drift}_{\mathrm{max}}$ (`{{MAX_DEAD_RECKONING_DRIFT_M_S}}` m/s).

---

### 8.5 Physical Spatial Constraints & Deployment Envelopes
- **Operational Staging Footprint:** Nominal cleared deployment and staging zone area bounded by $A_{\mathrm{staging}} \ge A_{\mathrm{staging\_min}}$ (`{{MIN_STAGING_AREA_M2}}` m²).
- **Transit & Packaged Packaging:** Entire cyber-physical platform, operator station, telemetry mast, and Ground Support Equipment (GSE) packaged into modular ruggedized containers with total combined volume $V_{\mathrm{packaged}} \le V_{\mathrm{packaged\_max}}$ (`{{MAX_PACKAGED_VOLUME_M3}}` m³) and single container mass $m_{\mathrm{container}} \le m_{\mathrm{container\_max}}$ (`{{MAX_CONTAINER_MASS_KG}}` kg) compliant with MIL-STD-1472H two-person lift constraints.
