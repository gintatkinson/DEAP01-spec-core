...
from scripts.lint_subagent_prompt import (
    ...
    check_m2_metamodel_directive,
    is_upstream_compiler_prompt,
)
...
class TestSubagentPromptLinter(unittest.TestCase):
    ...
    @staticmethod
    def _build_prompt(checklist_rows, classification="UPSTREAM_SPEC_CORE_COMPILER", extra_directives=None):
        checklist = "\n".join(f"- {row}" for row in checklist_rows)
        m2_directive = ""
        if "UPSTREAM_SPEC_CORE_COMPILER" in classification:
            m2_directive = (
                "5. Tier-1 Metamodel Transformation Mandate: Upstream compiler operates exclusively on abstract M2 metamodels "
