...
def _get_valid_conops_content() -> str:
    ...
    lines.append("            Containment[\"Autonomous Physical<br/>Containment Device\"]")
    lines.append("        end")
    lines.append("    end")
    lines.append("")
    ...
    lines.append("| :--- | :--- | :--- | :--- | :--- |")
    lines.append("| `UCL-01` | System Supervisory Operator | Mission initialization, profile authorization, and real-time supervisory tracking | Level 3 Certified Autonomous Systems Operator | Primary HITL Supervisory Monitor / Override |")
    lines.append("| `UCL-02` | Range Safety Officer | Range containment enforcement, emergency abort initiation, and corridor clearing | Level 4 Certified Range Safety Authority | Independent Flight Termination Authority |")
    lines.append("| `UCL-03` | Payload / Sensor Specialist | Payload sensor calibration, real-time data stream parsing, and telemetry offload | Level 2 Sensor System Specialist | Informational / Payload Control |")
    lines.append("| `UCL-04` | Maintenance Technician | O-Level / I-Level pre-flight BIT verification, battery hot-swap, and diagnostics | Certified Maintenance Engineer (CME) | Maintenance Mode Authority |")
    lines.append("| `UCL-05` | Systems Integration Engineer | Cross-subsystem interface verification, schema compliance auditing, and calibration | Senior Systems Engineer | Engineering & Calibration Authority |")
    lines.append("| `UCL-06` | Certification & Safety Auditor | Regulatory compliance inspection, safety case audit, and qualification assessment | Independent Safety Auditor | Audit & Verification Authority |")
    lines.append("| `UCL-07` | Ground Support Crew | Physical placement, charging interface connection, and mechanical tie-down | Certified Ground Operations Specialist | Physical Handling Support |")
    lines.append("| `UCL-08` | Telemetry Link Administrator | Frequency deconfliction, cryptographic key distribution, and link monitoring | Certified RF Network Administrator | Communications Infrastructure Role |")
    lines.append("")
    ...
    lines.append("        Environment[\"Atmospheric and<br/>Environmental Dynamics\"]")
    ...
    lines.append("        direction TB")
    lines.append("        GCS[\"Ground Control Station (GCS)<br/>• PORT-GCS-C2 (INOUT)<br/>• PORT-GCS-DISP (OUT)\"]")
    lines.append("    end")
    lines.append("")
    lines.append("    subgraph Platform_Segment[\"Air Vehicle and Primary Platform Segment (DoDAF SV-1)\"]")
    lines.append("        direction TB")
    lines.append("        subgraph Tier1_Processing[\"Guidance and Perception Tier\"]")
    lines.append("            direction TB")
    lines.append("            FCS[\"FlightGuidanceController<br/>(Flight and Guidance)<br/>• PORT-FCS-C2 (INOUT)<br/>• PORT-FCS-CMD (OUT)<br/>• PORT-FCS-TLM (IN)\"]")
    lines.append("            NavSensors[\"PerceptionFusionSubsystem<br/>(Sensors)<br/>• PORT-NAV-RF (IN)<br/>• PORT-NAV-DATA (OUT)\"]")
    lines.append("        end")
    lines.append("")
    lines.append("        subgraph Tier2_Actuation[\"Energy - Actuation and Safety Tier\"]")
    lines.append("            direction TB")
    lines.append("            BMS[\"BatteryManagementSystem (BMS)<br/>• PORT-BMS-PWR (OUT)\"]")
    lines.append("            Actuators[\"ActuatorSubsystem (Actuators)<br/>• PORT-ACT-IN (IN)\"]")
    lines.append("            Watchdog[\"SafetyWatchdog<br/>• PORT-WD-IN (IN)<br/>• PORT-WD-TRIG (OUT)\"]")
    lines.append("        end")
    lines.append("    end")
    lines.append("")
    lines.append("    subgraph Support_Segment[\"Launch and Auxiliary Support Segment (IEEE 1362 §5.3)\"]")
    lines.append("        direction TB")
    lines.append("        GSE[\"GroundSupportEquipment and Staging<br/>• PORT-GSE-PWR (OUT)\"]")
    lines.append("    end")
    lines.append("")
    lines.append("    Operator -->|\"CONN-01: Operator Command Input\"| GCS")
    lines.append("    GNSS_Space -->|\"CONN-02: L-Band RF Navigation Signals\"| NavSensors")
    ...
    lines.append("    GCS <-->|\"CONN-05: PACE Bidirectional C2 Datalink\"| FCS")
    lines.append("    NavSensors -->|\"CONN-06: Navigation State Estimates\"| FCS")
    lines.append("    FCS -->|\"CONN-07: Real-Time Actuator Demand Vector\"| Actuators")
    lines.append("    FCS -->|\"CONN-08: Heartbeat Pulse and Safety Telemetry\"| Watchdog")
