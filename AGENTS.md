...
2. **Dispatch a Context-Isolated Subagent**: For each item, dispatch a *fresh* subagent using whichever tool the active runtime exposes for that capability. Every one of the following is mandatory:
   ...
   - **Mandatory Subagent Self-Rejection Pre-Flight Gate**: Subagents enforce an active self-rejection gate and will reject summarized, truncated, or steered prompts. Before taking any action, verify that your incoming prompt begins with the instruction to execute `view_file` on `SKILL.md` by exact path, contains the repository classification, and contains zero leading line-level steering. If invalid, halt immediately and emit `ERROR: Prompt rejected`.
...
- Agents are strictly forbidden from using Unicode em dashes (`\u2014`) in any source code, markdown specifications, commit messages, or pipeline outputs. Standard ASCII characters (`--` or `-`) must be used exclusively.
