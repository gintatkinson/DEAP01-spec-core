...
flowchart TB
    subgraph SpaceSegment["Space & Satellite Segment"]
        GNSS["GNSS Constellation\n(GPS / Galileo / BeiDou)"]
        SATCOM["Iridium Satellite Network\n(Emergency SBD Link)"]
    end

    subgraph AirSegment["Air Vehicle Node (UAS Archetype)"]
        AV_FC["Flight Control Computer (FCC)\n(DO-178C DAL-A Autopilot)"]
        AV_SENS["Sensor Array\n(Triple IMU / Baro / Radar)"]
        AV_PAYLOAD["EO/IR Gimbal Payload\n(Edge Neural Processor)"]
        AV_COMM["PACE Datalink Transceiver\n(COFDM / Cellular / FHSS)"]
        AV_FTS["Flight Termination System\n(Independent Parachute Watchdog)"]
    end

    subgraph GroundSegment["Ground Control Station (GCS) Node"]
        GCS_C2["C2 Mission Console\n(STANAG 4586 DLI / CCI)"]
        GCS_ANT["Tracking Antenna Mast\n(Dual-Band COFDM / 4G)"]
        GSE_BAT["Ground Support Equipment\n(Battery Charging & BIT Rig)"]
    end

    subgraph UserSegment["Operational Human Roles"]
        RPIC["Remote Pilot in Command (RPIC)\n(Supervisory Authority)"]
        PO["Payload Operator (PO)\n(Sensor Tasking & Analysis)"]
        MC["Mission Commander (MC)\n(Sortie & ROE Release)"]
        MT["Maintenance Technician (MT)\n(O-Level & Pre-Flight BIT)"]
        VO["Visual Observer (VO)\n(Airspace Surveillance)"]
    end

    subgraph ExternalSegment["External Operational Entities"]
        UTM["U-Space / UTM Service Provider\n(Dynamic Geo-Zones & Tracking)"]
        ATM["Civil Air Traffic Control (ATC)\n(Emergency Airspace Coordination)"]
        EMERG["First Responders & Safety Services\n(Emergency Response Plan ERP)"]
    end
